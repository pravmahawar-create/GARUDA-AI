...
async function projectsHandler(req, res) {
  const customerSession = await currentCustomer(req);
  if (!customerSession || !customerSession.customer) {
    return res.status(401).json({ success: false, message: "Authentication required" });
  }
  const persistentProposalService = require("../src/services/persistentProposalService");
  const email = customerSession.customer.email;
  const projects = await persistentProposalService.listCustomerProjects(email);
  return res.status(200).json({ success: true, projects });
}

async function customerProposalsHandler(req, res) {
  const customerSession = await currentCustomer(req);
  if (!customerSession || !customerSession.customer) {
    return res.status(401).json({ success: false, message: "Authentication required" });
  }
  const persistentProposalService = require("../src/services/persistentProposalService");
  const email = customerSession.customer.email;
  const proposals = await persistentProposalService.listCustomerProposals(email);
...
const HANDLERS = {
  ...
  proposals: customerProposalsHandler
};
