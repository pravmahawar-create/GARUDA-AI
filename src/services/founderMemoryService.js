...
async function readStore() {
  if (supabaseConfigured()) {
    ...
    } catch (err) { console.warn("[auto-recovery] suppressed error in founderMemoryService.js:", String(err.message).slice(0,80)); }
  }
  try {
...
function buildPipelineSummary() {
  ...
  const extras = [];
  try {
    const { getPipeline } = require("./leadgen/genericLeadGenEngine");
    const tp = getPipeline({ domain: "tutoring" });
    if (tp && tp.total) extras.push(`Tutoring (sister): ${tp.total} prospects generated by web-scan.`);
  } catch (err) { console.warn("[auto-recovery] suppressed error in founderMemoryService.js:", String(err.message).slice(0,80)); }
  try {
    ...
    if (scan && scan.jobId) {
      extras.push(
        `Tutoring web-scan ${scan.jobId}: ${scan.running ? "RUNNING" : "done"}, ${scan.scanned || 0} sites scanned, ${scan.emailsFound || 0} emails found.`
      );
    } else {
      extras.push("Tutoring web-scan: abhi tak start nahi hua. Founder command: 'tutoring leads usa' ya 'tutoring leads dubai'.");
    }
  } catch (err) { console.warn("[auto-recovery] suppressed error in founderMemoryService.js:", String(err.message).slice(0,80)); }
  const extrasBlock = extras.length ? "\n" + extras.join("\n") : "";
  return `- ${sent} emails delivered (${bounced} bounced/dead addresses excluded, ${interested} interested replies tracked). Pipeline file-based, real outreach LIVE.${extrasBlock}`;
}
