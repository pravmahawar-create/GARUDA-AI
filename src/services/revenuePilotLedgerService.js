...
function buildLedgerEntry(requestInput, input = {}, previousEntryHash = null, now = new Date()) {
  ...
  if (request.completionReceipt?.verificationMethod !== "signed_provider_webhook" || !request.completionReceipt?.payloadHash || !request.completionReceipt?.signatureHash) fail("Signed provider webhook evidence is required", 409);
...
function deploymentReadiness(env = process.env) { const nodeEnv = String(env.NODE_ENV || "development"); const publicUrl = String(env.GARUDA_PUBLIC_URL || ""); let publicUrlValid = false; try { publicUrlValid = new URL(publicUrl).protocol === "https:"; } catch (_) {} const payment = require("./revenuePaymentReceiptService").accountReadiness(env); const checks = { productionMode: nodeEnv === "production", publicHttpsUrl: publicUrlValid, databaseConfigured: Boolean(env.MONGODB_URI), connectorExplicitlyConfigured: env.GARUDA_WEBHOOK_ENABLED === "true", eligiblePaymentAccountConfigured: payment.ready }; return { ready: Object.values(checks).every(Boolean), checks, externalDispatchDefaultOff: env.GARUDA_WEBHOOK_ENABLED !== "true", livePaymentInitiationEnabled: false, truth: Object.values(checks).every(Boolean) ? "Deployment and eligible payment receipt verification are configured; external delivery still requires Founder approval." : "Deployment is not production-ready until every configuration and eligible payment-account check passes." }; }
