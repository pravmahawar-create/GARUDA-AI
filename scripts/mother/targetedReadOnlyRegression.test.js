...
(async () => {
  ...
  const [result] = await execute([{ id: "targeted-read-only", task: request }]);
  ...
  const [missingResult] = await execute([{ id: "missing-target", task: missingRequest }]);
  ...
  console.error("Targeted read-only regression FAILED:", err);
  process.exitCode = 1;
});